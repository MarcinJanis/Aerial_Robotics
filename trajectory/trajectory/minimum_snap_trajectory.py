...
class MinimumSnapTrajectory(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(
            PolynomialTrajectory, ROS2_topic_name_Trajectory, 10
        )

        # Wyznaczenie płynnej trajektorii globalnej metodą Minimum Snap
        self.trajectory_coefficients, self.global_time_vect, self.local_time_vect = (
    ...
    def set_output(self):
        ...
        self.get_logger().info(
            f'Seg: {segment_idx}, T_seg: {segment_duration:.2f}s, Global t: {global_t:.2f}s'
        )
    ...
    def find_trajectory(self):
        ...
        for axis in range(3):
            ...
            r_ve = np.zeros(8 * M)
            for i in range(1, 8):
                r_ve[8 * (M - 1) + i] = i / T_end
            A_list.append(r_ve)
            b_list.append(0.0)
            # Przyspieszenie końcowe = 0
            r_ae = np.zeros(8 * M)
            for i in range(2, 8):
                r_ae[8 * (M - 1) + i] = i * (i - 1) / (T_end**2)
            A_list.append(r_ae)
            b_list.append(0.0)
            # Szarpnięcie (jerk) końcowe = 0
            r_je = np.zeros(8 * M)
            for i in range(3, 8):
                r_je[8 * (M - 1) + i] = i * (i - 1) * (i - 2) / (T_end**3)
            A_list.append(r_je)
            b_list.append(0.0)
            ...
            for k in range(M - 1):
                ...
                r_sc = np.zeros(8 * M)
                for i in range(4, 8):
                    r_sc[8 * k + i] = i * (i - 1) * (i - 2) * (i - 3) / (T_k**4)
            ...
            try:
                ...
                self.get_logger().error(
                    f'[Trajectory Error] Nie można rozwiązać układu KKT dla osi {axis}'
                )
                c_opt = np.zeros(8 * M)
